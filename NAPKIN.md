...
**Extensible data inputs:** The harness tests any data point that might improve prediction accuracy — trending topics, competitor activity, time of day, media type, hashtag patterns, reply context. New data sources are A/B tested against real outcomes. If a data point improves accuracy, it stays.

## Key decisions

- **Probability, not exact numbers.** The harness outputs "70% chance of hitting 10k in 24h," not "this will get exactly 8,200 impressions." Probabilities fit Brier score eval naturally and are more useful for go/no-go decisions.
...
- **Live eval on real tweets.** The harness is evaluated against real tweets captured in real-time via X API filtered stream. The harness predicts, the impressions resolve naturally, predictions are scored. No sandbox, no historical replay — the harness gets the same data a real marketing agent would have.
- **The harness is not the product.** PostProphet is the prediction layer. Tweet generators, autonomous posting tools, marketing dashboards — those are products built on top.
- **Generalizes beyond X.** Start with X, but the concept applies to any platform with engagement metrics and an API.

## MVP scope

**In:**
- Prediction harness: takes who + what + where + target + timeframe → outputs probability + reasoning
...
- Regression testing: PRs to the harness are scored against the same tweet batch, does accuracy improve?
